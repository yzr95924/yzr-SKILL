...
- **Iron Law（没观察到失败就别写 skill）**：写纪律型 / 模式型 / 参考型 skill 前必须先**不带 skill
  跑 2–3 个典型 prompt**，把 agent 的违规与借口（"为简化" / "用户没说明" / "这样更快" / "应该等价"）
...
- **避免太多选项**：每个决策点给"1 个推荐 + 1 个 escape hatch"，不列 N 个等价选项——N 个等价选项
  逼 agent 自己判断，反而拖慢决策。
- **executable scripts 优于生成代码**：可脚本化且反复需要的操作写成脚本，别让 agent 现场生成——
  省 token（输出占 context、源码不占）、保证一致性。
...
| 正文描述一致性 | 核心原则关键词 grep 全部 `.md` | ≥ 2 次且无 self-aware 注释 = 重抄嫌疑，读段对比确认 |
