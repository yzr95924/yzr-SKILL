...
2. **不带 skill 跑 2–3 个典型 prompt 观察失败（RED 阶段）**——细节与原则见
   「创建一个 skill · baseline 演练（RED 阶段）」；**纯参考资料型 skill 可跳过**
...
7. 重复上述过程，直到满意为止
8. **收敛后扩量再验证（防过拟合的最后一道闸）**：2–3 个 prompt 反复迭代收敛后，把测试集扩到 5–10 条（覆盖更广的意图类别 + 相邻负例），再跑一轮完整评估——小样本收敛 ≠ 大样本成立，扩量能暴露"过拟合到少数 case"的残留问题
...
写完 skill 草稿后，设计 2–3 个测试 prompt——真实用户实际会说的话
