...
description: |
  ...
  - **Optimize description only**: 只想动 frontmatter description——用户说"优化描述"是泛指时走入口 1/2（见正文「描述层（trigger vs 用户语境）」）
...
- **描述层（trigger vs 用户语境）**：两个维度分清别串——
  - (a) **skill 设计**：frontmatter `description` 只决定"何时调"，正文决定"怎么用"；入口 3 只动 frontmatter，入口 1/2 才动正文
  - (b) **用户语境**：用户说"优化 XX 的描述"是泛指——默认包括 frontmatter `description` 字段 + 标题 + 章节 + when-to-use 措辞 + 操作步骤说明，不要默认专指 frontmatter；用户要细分会用精确措辞（"只改 frontmatter" / "只动 description 字段" / "标题层调整"等）
  - 把 (a) 当 (b) 的依据 = 默认把"优化描述"窄化成只动 frontmatter，违背 (b)
...
- **三件套豁免声明**：yzr-skill-creator 是元 / pattern skill——它要求别的纪律型 skill 必须带
  Rationalization Table + Red Flags + 违反字面=违反精神三件套，自己不内嵌：
  (1) 它的禁令都是引用到 references 的具体操作约束（`cp -r` 旧版 / 同轮并行 baseline /
  `CONST` 引用代码风格等），不是抽象禁令——不需要三件套堵漏洞；
  (2) 它不面对"agent 压力下偷懒"的场景——读者是来学方法论的，不是来跑禁令的
...
跑过测试用例、用户评审过结果后，根据反馈迭代——核心 4 条（从反馈归纳泛化 / 保持精简 /
解释"为什么"而非堆砌 MUST / 找跨用例重复工作进 `scripts/`）全部展开在
`references/skill-writing-principles.md`「精简原则」+「解释为什么」+
「executable scripts 优于生成代码」三条下，此处不重抄。
...
理解触发机制有助于设计更好的评估查询——详见 `references/description-trigger-mechanics.md`。
...
- `references/description-trigger-mechanics.md` —— skill 触发原理（写触发评估查询前先读）
