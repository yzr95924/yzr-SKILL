...
<wiki-root>/
├── .gitignore
├── AGENTS.md                  # 0.11.0+：工具无关纪律 SSOT（详 §2）；顶部
│                              # @MEMORY/MEMORY.md + @scripts/SCRIPTS.md 双
│                              # @import 收口（0.24.0+；0.23.0 短改内联已回退）
...
│                              # @MEMORY/MEMORY.md @import 自动加载（0.24.0+；
│                              # 详见 §5.1）
├── raw/
│   ├── articles/
│   └── assets/
...
│                              # @scripts/SCRIPTS.md @import 自动加载（0.24.0+；
│                              # 详见 §14.3）
└── wiki/
    ├── comparisons/
    ├── concepts/
...
- **`MEMORY/` 是 wiki 仓的"agent 持久化记忆"目录**（0.10.0+ 起与 `wiki/` **平级**，
  ...
  `@MEMORY/MEMORY.md` `@import` 自动加载全文——Claude Code 经薄壳 `CLAUDE.md → @AGENTS.md`
  递归展开 / Qoder 原生支持 `AGENTS.md` 内 `@MEMORY/...` import 透明拿到索引；Codex
  不展开 `@import`，由 AGENTS.md 项目记忆段 HTML 注释指引直接 Read 拿到。0.24.0+ 恢复
  `@import` 收口（0.23.0 短改内联因双写 / 词数膨胀等坑已回退，详见 §5.1 加载机制段）——
  AGENTS.md 只挂单行引用、不持有副本，**无**双写漂移 / L1 膨胀风险。**为什么移到
...
>
> **0.24.0+**：`AGENTS.md` 顶部含 `@MEMORY/MEMORY.md` + `@scripts/SCRIPTS.md` 两行 `@import` 收口
> ——Claude Code 经薄壳 `CLAUDE.md → @AGENTS.md` 递归展开 / Qoder 原生支持 `AGENTS.md` 内 `@import`
> 透明自动拿到 MEMORY / scripts 全文；Codex 不展开 `@import`（openai/codex#17401 未实现），
> 由 `AGENTS.md` 顶部 HTML 注释显式 Read 指引补回。**MEMORY / scripts 是单一真源**，AGENTS.md
> 单行 `@import` 引用同步指向全文——**不**在 AGENTS.md 持有副本，**无**双写漂移 / L1 膨胀风险
> （0.23.0 内联方案踩过的坑）。详见 §5.1 + §14.3 加载机制段。
>
> **0.23.0+** 内联方案已废——曾把 `@MEMORY/MEMORY.md` / `@scripts/SCRIPTS.md` 删掉、把内容
> 内联进 `AGENTS.md` §一两个 h4 段（论证：当时认为 `@import` 只 Claude Code 展开，故作此补救）；
> Qoder 现确认原生支持 `@import`，故 `@import` 收口现在 2/3 家直接透明、Codex 靠 HTML 注释指引
> 兜底——比内联方案简单得多（详见 `yzr-multi-agent-context/SKILL.md`「设计与原理」段）。
>
...
> 同时在 AGENTS.md 顶部插入 `@scripts/SCRIPTS.md` import 行（0.24.0+ `import` 收口恢复）。
...
- 不含纪律正文；AGENTS.md 顶部 `@MEMORY/MEMORY.md` / `@scripts/SCRIPTS.md` `@import`
  由 AGENTS.md 自身携带（0.24.0+ 收口），薄壳内**不**再额外挂 import；仅 `@AGENTS.md` 一行
...
- **`MEMORY/MEMORY.md`（索引）——CLI init 时刻写入**（fixtures 字面量见
  ...
  `@MEMORY/MEMORY.md` `@import` 加载到所有读 AGENTS.md 的 agent（0.24.0+；
  0.23.0 短暂改内联因双写 / 词数膨胀坑已回退，详见 §5.1 加载机制段）。详见 §5.1
...
- **无 frontmatter**——它是**被 `<wiki-root>/AGENTS.md` 用 `@MEMORY/MEMORY.md` `@import` 加载**
  的索引片段（0.24.0+ 回归 `@import` 收口；0.23.0 短暂改内联——存在双写 / L1 词数膨胀 / 真源分裂
  等问题已回退，详见 `yzr-multi-agent-context/SKILL.md`「为何不再用内联」段），不是 wiki 内容页
...
- **加载机制（agent 中立；0.24.0+ `@import` 收口）**：`<wiki-root>/AGENTS.md` 顶部单行
  `@MEMORY/MEMORY.md` —— Claude Code 经薄壳 `CLAUDE.md → @AGENTS.md` 递归展开自动加载
  `MEMORY.md` 全文（SSOT），Qoder 原生支持 `AGENTS.md` 内 `@MEMORY/...` import 也自动展开；
  Codex 不展开 `@import`（openai/codex#17401 未实现），故 AGENTS.md 的「跨会话记忆（索引）」
  段同时挂一条面向 Codex 的 HTML 注释显式 Read `MEMORY/MEMORY.md` 指引。三家都见 L2 索引——
  `@import` 机制透明展开，**不**像 0.23.0 内联方案需要在 AGENTS.md 复制全部条目
- **不**引入内联条数护栏——索引只活在 `MEMORY/MEMORY.md` 单一真源，AGENTS.md 单行引用不占 L1
  词数预算；MEMORY 沉淀自由增长、AGENTS.md 不受影响。`INLINED_INDEX_MAX` 常量与
  `inlined-memory-index-bloating` warn 已删除（[lint-checklist §二.15](lint-checklist.md)
  整章移除）
...
- lint 校验（实现见 SKILL 仓 `scripts/lint_wiki.py`）：
  ...
  - `MEMORY/MEMORY.md`（索引）**不**参与此校验——本身无 frontmatter，是被
    `AGENTS.md` 用 `@MEMORY/MEMORY.md` `@import` 加载的索引片段（0.24.0+；
    0.23.0 短暂改内联已回退）
- 与 wiki 内容页的区别：
...
### §14.3 索引 `SCRIPTS.md`（AGENTS.md 顶部 `@import` 加载；0.24.0+）
...
- **加载机制（agent 中立；0.24.0+ `@import` 收口）**：`<wiki-root>/AGENTS.md` 顶部单行
  `@scripts/SCRIPTS.md` —— Claude Code 经薄壳 `CLAUDE.md → @AGENTS.md` 递归展开自动加载
  `SCRIPTS.md` 全文，Qoder 原生支持 `AGENTS.md` 内 `@scripts/...` import 也自动展开；
  Codex 不展开 `@import`，同 §5.1 顶部 HTML 注释指引补回去。三家都见 scripts 索引——
  `@import` 机制透明展开，**不**像 0.23.0 内联方案需要在 AGENTS.md 复制脚本条目
- **形态**：单段 `## 索引`（既是 AGENTS.md `@import` 加载源、又是 LLM agent 按需 Read 的入口），
  每脚本一段：`` - `<name>` — <一句话用途> `` one-liner 起头 + `### <name> — <label>` 子节
  含 4 要素（使用场景 / 调用约定 / 作用 / 可选前置依赖）。0.23.0 的「双段形态」（`## 索引` +
  `## 分节契约（详细）`）已合并回单段——`@import` 加载后 agent 全文 Read 即可，
  **不**需要在 AGENTS.md 单独投影 one-liner
- **更新纪律（0.24.0+）**：脚本增删时**只改这一份**（`scripts/SCRIPTS.md`）——AGENTS.md 单行
  `@import` 引用同步指向 `SCRIPTS.md` 全文，**不**需要再同步 AGENTS.md 投影（0.23.0
  「双处同步」已废）
- **骨架**(CLI init 时刻拷贝):
  ...
  >
  > **加载机制（0.24.0+）**：由 `<wiki-root>/AGENTS.md` 顶部 `@scripts/SCRIPTS.md`
  > 单行 `@import` 加载——Claude Code 递归展开 + Qoder 原生展开，Codex 由 AGENTS.md 顶部
  > HTML 注释指引直接 Read 本文件。脚本增删时**只改这一份**（详见上方 §14.3 加载机制说明）。
  >
...
### §14.4 每工具一段的契约(LLM / 用户共同维护；0.24.0+ 单段形态)
...
agent Read 时第一眼看到的 hook），下面紧跟 4 要素段落——只活一份，单段形态（与 0.23.0 双段区别：
不再区分 `## 索引` 段 vs `## 分节契约（详细）` 段，`@import` 加载后 agent 全文 Read 即可）。
...
| --- | --- | --- | --- |
| AGENTS.md 加载方式（0.24.0+ `@import` 收口） | 顶部 `@MEMORY/MEMORY.md` `@import` 自动展开 | 不内联——LLM 不需要看完整 tag 字典；按需 `Read` | 顶部 `@scripts/SCRIPTS.md` `@import` 自动展开 |
...
### §14.8 0.24.0 反向迁移（内联 → `@import`）

升级至 0.24.0 后，老 wiki（0.23.0 「内联方案」形态——`AGENTS.md` §一含
`#### 跨会话记忆（索引）` / `#### Wiki-local scripts（索引）` 段，不含顶部
`@MEMORY/MEMORY.md` / `@scripts/SCRIPTS.md` import）需由 LLM agent 走以下迁移步骤
（`lint_wiki.py --check-version --apply` 配合 `check_wiki_fixtures.py` 新增
`agents-md-has-at-imports` + `agents-md-codex-read-hint` check 自动发现）：

1. **删 `AGENTS.md` §一两个 h4 段**（`#### 跨会话记忆（索引）` 与 `#### Wiki-local scripts（索引）`）
2. **在 `AGENTS.md` 顶部加两行 `@import`**：`@MEMORY/MEMORY.md` + `@scripts/SCRIPTS.md`
3. **在 `AGENTS.md` 顶部加面向 Codex 的 HTML 注释 Read 指引**（Codex 不展开 `@import`，
   靠这条指引直接 Read `MEMORY/MEMORY.md` 拿到索引；详见 §5.1 加载机制段）
...
7. Edit `<wiki-root>/AGENTS.md` §八「Wiki Spec 版本」行改为 `0.24.0`
8. 重跑 `lint_wiki.py --check-version` 验证——`agents-md-has-at-imports` 应 pass，
   `agents-md-codex-read-hint` 应 pass
