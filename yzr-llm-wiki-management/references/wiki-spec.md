...
├── CLAUDE.md                  # 0.11.0+：薄壳（@AGENTS.md，供经薄壳加载的 agent）
...
>
> **0.25.0+**：`AGENTS.md` 顶部含 `@MEMORY/MEMORY.md` + `@scripts/SCRIPTS.md` 两行 `@import` 收口 +
...
>
> **0.23.0 内联方案已废**——曾把 `@MEMORY/MEMORY.md` / `@scripts/SCRIPTS.md` 删掉、把内容内联进
> `AGENTS.md` §一两个 h4 段；因双写漂移 / L1 膨胀 / 真源分裂已回退，改回 `@import` 收口 + 顶部
> 强制 Read 指令兜底（详见 `yzr-multi-agent-context/SKILL.md`「设计与原理」段）。
>
...
| 索引文件 SCRIPTS.md（0.9.0+） | 与 tags.md / MEMORY/MEMORY.md 同——**无** frontmatter，裸 Markdown；由 `<wiki-root>/AGENTS.md` 顶部 `@scripts/SCRIPTS.md` `@import` 加载（0.24.0+ 收口；0.23.0 短暂内联已回退） | `scripts/SCRIPTS.md` |
