...
## 九、交互语言风格（对用户不用黑话，严谨精确）

> **本段 SSOT 反指**：交互语言风格的权威定义在 SKILL.md §执行原则/边界 核心原则 §13；
> 本文件是 wiki 仓自带模板（workspace CLI init 时拷贝到目标 wiki 根，跨仓引不到 SKILL.md），
> 必须自包含。与 SSOT 措辞故意保持一致，改 SSOT 时同步改本段。
