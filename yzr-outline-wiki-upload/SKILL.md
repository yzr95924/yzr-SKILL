...
description: 用户要往 Outline Wiki 工作区写内容时使用——创建 / 编辑文档，以及扩展写
  ...
  **不**用于搜 / 读 outline 文档（走 yzr-outline-wiki-search）；
  **不**用于配置 outline MCP（走 yzr-outline-wiki-setup）；**不**用于 Notion / Confluence /
...
读 / 搜由 `yzr-outline-wiki-search` SKILL.md 负责；
MCP 配置由 `yzr-outline-wiki-setup` SKILL.md 负责。
...
- **配置 outline MCP**（首次接入 / 重启后看不到 / 401）——走
  `yzr-outline-wiki-setup` SKILL.md
- **写前 search 查重 / 定位**——也是 `yzr-outline-wiki-search` 范畴（search + read
...
启动时需具备以下**前置条件**——这些由 `yzr-outline-wiki-setup` 负责：

- **MCP 已注册**：当前 session 能调 `mcp__outline__*` 系列工具
  （若未注册，**先走 `yzr-outline-wiki-setup` SKILL.md**，
  跑完重启后再回来）
...
> `yzr-outline-wiki-search` 读回 markdown body、外部 OKF 消费端解析、检索分块都
...
create / edit 两个（search / read 由 `yzr-outline-wiki-search` SKILL.md
...
1. **写前先搜（Read-First）**
   - 创建新文档前必须先用 `yzr-outline-wiki-search` 查重，确认是否已有同类但
     内容过期
...
- **不**在 MCP 未启用时尝试操作（先走 `yzr-outline-wiki-setup`）
...
1. **核实配置与工具**：会话开始时——
   - 确认 outline 相关 MCP 工具在当前 session 已注册；若未注册，**先走
     `yzr-outline-wiki-setup` SKILL.md**，跑完重启后
     再回来
   - 调 MCP `tools/list` 取实际工具清单（核心能力 + 扩展能力各自对应的
     真实工具名、参数 schema）
2. **理解意图**：把用户的自然语言指令映射到能力清单之一（核心或扩展）
3. **先搜后写**（与 `yzr-outline-wiki-search` 协作）：涉及"创建 / 编辑"前，
...
2. **上传二进制**：用 `curl` 把本地文件 POST 到 `uploadUrl`（即 `/api/files.create`）。
   ...
   > 2. 重跑 `yzr-outline-wiki-setup/scripts/configure_mcp.py` 重新写 key
...
> `yzr-outline-wiki-setup` SKILL.md 负责。本 skill 假设
...
1. **认证失败（401 / 403）**
   - 这是 `yzr-outline-wiki-setup` 的范畴（key 过期 / OAuth 失效），先重启或
     重新跑配置脚本
2. **MCP 未启用（连接被拒）**
   - 走 `yzr-outline-wiki-setup` 排查配置
3. **工具名不匹配**
   - 重新调 `tools/list` 拿当前工具清单；不要凭记忆中的名字调用
4. **文档 ID 无效**
   - 用 `yzr-outline-wiki-search` 重新定位目标文档，拿到新 ID
...
```text
1. 用 yzr-outline-wiki-search 查 '后端' Collection ID（若 create schema 要求）
...
```text
1. 用 yzr-outline-wiki-search 读 doc_abc123 拿当前正文
...
```text
1. 用 yzr-outline-wiki-search 查目标 Collection + 是否已有同名文档
...
4. 用 yzr-outline-wiki-search 的 read 拿当前 summary.md 中所有
...
- `yzr-outline-wiki-setup` SKILL.md — 配套：MCP 配置与首次接入
