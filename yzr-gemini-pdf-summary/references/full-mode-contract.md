...
>
> - **paper full**（`--type paper --full`，**2026-07-07 翻面**：从"双产物 quick + full"翻为"单产物 full"，
>   与 manual / whitepaper / book 单产物形态对齐）
...
> **TOC 锚点说明（2026-07-07 防护加固）**：本目录所有锚点指向**显式 HTML anchor**（下方
> 每个 `## ` 前的 `<a id="..."></a>`），不是 GitHub 自动生成的 slug。heading 文本后续
> 即使改名 / 加标注 / 改全角符号，TOC 跳转仍然稳定。改 heading 时**不要**改对应 `<a id>`，
> 除非是有意删节。

- [接口约定核心](#interface-contract)
- [调用形式](#invocation-forms)
- [产物 layout](#output-layout)
- [by-chapter 模式](#by-chapter-mode)
- [关键纪律](#key-discipline)
- [关键行为护栏](#behavior-guardrails)
- [反模式](#anti-patterns)
- [专属故障排查](#troubleshooting)
...
### paper `--full`（单产物，2026-07-07 翻面）

**2026-07-07 翻面**：原设计"单次调用同时产 quick summary + full 双产物"翻为"**单产物 full**"——
与 manual / whitepaper / book 的 full 风格产物形态对齐，4 类 full 统一"单文件 + LLM 消费底座"。

单次调用产 **一份 PDF→Markdown 全量转写** 产物；产物 layout **强制 raw-compatible**——
`--output` 视为 wiki 仓根，直接落到 `<wiki_root>/raw/papers/<slug>.full.md`（**不写 `.quick.md`**）。
**不要**用 `--full` 把产物落任意目录——那是 `--extract-figures` 的职责。

**想看 quick summary + 带图**：跑 `--type paper` 默认（不加 `--full`），quick 模式产 `<output>/summary.md` +
`figures/*.png`。
**想看 PNG**：跑 quick 模式（默认带图）或 `--extract-figures` 单跑。full 模式不再落 PNG，
也不再跑 Stage 2；Stage 2 仅 quick 模式隐式行为，`--refine-figures` 仅作用于 quick 模式 +
`--extract-figures`。

**2026-06-30 第二轮翻面**：full 模式不落 PNG；本次翻面（2026-07-07）进一步不产 quick。
...
python3 yzr-gemini-pdf-summary/scripts/gemini_pdf_summary.py \
...
| --- | --- | --- | --- |
| **paper `--full`** | `<wiki_root>/raw/papers/<slug>.full.md`（单产物，**2026-07-07 翻面后**） | 否（full 自包含；quick 模式另产 PNG 到 `raw/assets/<slug>/`） | 显式 `--full`；默认拒绝覆盖，加 `--force-full` |
...
任何"占位 source 页"或后续蒸馏由消费端 skill（`yzr-llm-wiki-management`）自行处理，
本 skill 不碰 `wiki/` 目录。

---

<a id="by-chapter-mode"></a>
...
> 98 页华为 OceanDisk 白皮书单次调用 → 35%+ 章节截断）。
...
| token 上限 | L1 模式每章 ≤ 50 页，撞 `FULL_MAX_OUTPUT_TOKENS` 上限概率近 0；auto 模式仍受 `FULL_MAX_OUTPUT_TOKENS` 上限约束 |
...
| 与 `--full` 关系 | **互斥**——`--by-chapter` 已隐含全文级转写；`--full` 是 paper 单产物路径 |
...
| 维度 | manual / whitepaper / book full | paper `--full`（2026-07-07 翻面后） | **by-chapter L1**（默认） | **by-chapter auto**（旧） |
| --- | --- | --- | --- | --- |
| 调用次数 | 1 | **1（**翻面前是 2） | **N_L1** | 1 |
| 产物形态 | 1 个 .md | **1 个 .md**（翻面前是 2 个：quick + full） | **N_L1 + 1 个 .md**（N 个 L1 + 1 个 TOC） | **N + 1 个 .md**（N 章 + 1 个 TOC） |
| 章节粒度 | 全文级单文件 | 全文级单文件 | **逐 L1 文件 + L2 合并** | **逐章独立文件** |
| 适用场景 | 普通全文转写 | paper LLM 消费底座（与 manual/whitepaper/book 对齐） | **llm-wiki 多次 ingest / 逐章 Q&A（长 PDF）** | **短 PDF 单次过** |
| token 上限 | `FULL_MAX_OUTPUT_TOKENS` | `FULL_MAX_OUTPUT_TOKENS` | **每章 < 50 页几乎不撞** | **`FULL_MAX_OUTPUT_TOKENS`（更紧：JSON 格式增加 ~3% 开销）** |
...
by-chapter 模式受 `FULL_MAX_OUTPUT_TOKENS`（SSOT 脚本常量；撞顶阈值见 `scripts/gemini_pdf_summary.py`）上限约束。脚本内置两层检测：
...
**L1 模式**（默认）：每章 ≤ 50 页 → 撞 `FULL_MAX_OUTPUT_TOKENS` 上限概率近 0；如有截断，WARN
...
### paper `--full` 专属纪律（2026-07-07 翻面）

**2026-07-07 翻面**：原"单次调用必须产两份"的设计被取消，paper `--full` 现在与 manual /
whitepaper / book 一样走"单产物 full"。理由：

1. **产品形态对齐**：4 类 full（paper full / manual / whitepaper / book）现在统一是
   "**单文件 + LLM 消费底座**"。下游 `yzr-llm-wiki-management` 的 ingest 路径把 full.md
   当 source 蒸馏，quick summary 与 full 是不同消费对象——quick 给**人**看、full 给 LLM
   消费，分两次跑比"塞一份调用里"更符合"产物定位分家"的产品语义。
2. **下游 quick 需求解耦**：如果未来需要 quick summary + PNG，沿用 `--type paper` 默认
   跑一次（quick 模式独立产 `<output>/summary.md` + `figures/*.png`），不强制与 full 同绑定。
3. **避免"贵读一次"重复**：用户要 quick 跑 quick 模式、要 full 跑 `--full` 模式，独立触发，
   不会因"跑 `--full` 就自动多走一遍 quick"造成无谓 token 开销。

- **`raw/papers/<slug>.full.md` 不重写**：默认拒绝覆盖；下游已多次引用 full 抽取，
  意外重写会丢下游状态。若要重新抽取，先 `rm <wiki-root>/raw/papers/<slug>.full.md`
  再跑
- **想看 quick + 带图**：跑 `--type paper` 默认（quick 模式），产物 `<output>/summary.md` +
  `figures/*.png`；不要试图加 `--full` 又拿到 quick 产物
- **想看 PNG**：跑 quick 模式（默认）或 `--extract-figures` 单跑
...
- **Output token 上限**：`FULL_MAX_OUTPUT_TOKENS` 模块常量（`scripts/gemini_pdf_summary.py`）。
  4 类 full 都传该值。quick 模式走默认 token 上限
...
### paper `--full` 专属反模式（2026-07-07 翻面后）

- 跑 `--full` 但希望产物落 `<dir>/papers/<slug>.full.md` 而非 `raw/papers/...`——**不会发生**，
  `--full` 强制 raw-compatible；想落任意目录就**不**加 `--full`，直接跑 paper quick 默认（就带图）
- 跑 `--full` 后又跑 quick 模式凑两份产物——这是 2026-07-07 翻面后的**正确用法**（quick 与 full
  独立触发、各走各的产物 layout），不要再试图靠 `--full` 一份调用出两份
- 跑 `--full` 但产物里出现 `![图 N](PDF p.X ...)` 引用——full 模式不落 PNG，引用会断图；
  脚本 `strip_pdf_figure_refs` 在 main 末尾清理；如出现说明 prompt 漏改
...
| 跑 `--full` 期望拿到 quick summary | **预期外**：2026-07-07 翻面后 `--full` 不产 quick | 改跑 `--type paper` 默认（quick 模式）拿 quick summary + 带图 |
