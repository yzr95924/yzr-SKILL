...
- 产物最终要落到 `yzr-llm-wiki-management` 的 `raw/<type>/<slug>/` 下做后续 ingest
...
>   后续 `yzr-llm-wiki-management` 的 `ingest_diff.py` 找不到产物就白跑。
...
### D. 与 yzr-llm-wiki-management 的接力
...
3. yzr-llm-wiki-management：跑 ingest_diff.py 发现 raw 新增
