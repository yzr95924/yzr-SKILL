...
本脚本是 yzr-gemini-pdf-summary skill 的核心可执行入口。
...
def _run_full_mode(args):  # type: (argparse.Namespace) -> int
    ...
    关键决策 SSOT:../../MEMORY/yzr-gemini-pdf-summary-paper-full-single-output.md
...
def main(argv=None):  # type: (Optional[list]) -> int
    ...
    if args.auto_detect:
        try:
            ...
            sys.stderr.write(
                ...
                "       请确认 yzr-gemini-pdf-summary/scripts/auto_detect.py 存在。\n"
            )
            return 2
        try:
    ...
    # ../../MEMORY/yzr-gemini-pdf-summary-paper-full-single-output.md（D1）
