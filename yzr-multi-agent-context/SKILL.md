...
为什么不沿用之前的「把 MEMORY.md 索引内联进 AGENTS.md 正文」方案：

- **双写漂移**：MEMORY.md 加一条就要去 AGENTS.md 再贴一次 → 必不一致，靠 `check_memory_sync` 守门
  是补救而不是机制。
...
3. 应用 R1（去品牌）+ R3（行宽）。
4. 控制 L1 正文词数（预算见 [`references/layering.md`](references/layering.md)）；超出把详细内容
   下沉到 L2（`MEMORY/`）。记忆索引走 `@import`，**不**计入 AGENTS.md 词数预算（只占 1 行引用 + 1 段指引）。
5. 自检：
