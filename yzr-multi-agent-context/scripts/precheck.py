...
def check_project(root: Path) -> Tuple[List[str], bool]:
    ...
    if has_agents:
        ...
        if has_claude:
            ...
            lines.append("  [路径 2] 已有 AGENTS.md（纯，无 CLAUDE.md）→ 规范化（去品牌残留 / 补 L2 内联）")
    elif has_claude:
    ...
    if memory.is_dir():
        ...
        lines.append("  [INFO] 新规范：L2 记忆索引须内联进 AGENTS.md 正文（## 跨会话记忆 段落），")
        lines.append("         旧 @MEMORY/MEMORY.md import 已废弃（Codex / Qoder 不展开 import）")
    else:
