...
结构模板与切换规则见 [章节](#step-4-形态路由)–[章节](#step-6-报告形态)

## 执行原则

1. **不主动改文件**: 产出是结论 / 报告,用户点头后才走具体重构
2. **每条发现可追溯**: 每条发现映射到至少 1 个 catalog 卡片名
...
LLM 用 catalog 的重构卡片 + 合理性审视卡片补齐设计层问题。
数值阈值项(函数长度 / 嵌套层数 / 参数个数 / 重复块规模等)先现场 grep / AST 速算拿客观计数,
再进判断层定严重度,不凭目测;阈值数值只在 `ref/severity-rubric.md` 给
...
3. **列要点**: 按严重度从高到低;发现 ≤ 3 条直接给全(位置 + 卡片名 + 理由 + 建议);> 3 条给 top 概览
4. **收尾问询**: 发现多时问"逐条过一遍还是出一份完整档报告";逐条过按严重度从高到低逐条呈现,每条等用户表态:
   - **确认** → 记为"接受",下一条
...
开头先给一段**整体理解**(口径同 [章节](#step-5-对话式分析回答默认) 第 1 步,不超过一段),再按 `ref/report-template.md`
