...
必读 `ref/catalog.md`(规则清单);LLM 用其中的重构规则 + 合理性审视规则补齐设计层问题。
数值阈值项(函数长度 / 嵌套层数 / 参数个数 / 重复块规模等)先现场 grep / AST 速算拿客观计数,
再对照规则内的阈值判断是否成发现项,不凭目测

### Step 3: 形态路由

默认进 **对话式分析回答**(Step 4);用户明确要报告或大范围体检(多文件 / 遗留代码)→ 进 **报告形态**(Step 5);形态可中途切换
...
开头先给一段**整体理解**(口径同 [章节](#step-4-对话式分析回答默认) 第 1 步,不超过一段),再按 `ref/report-template.md`
两档输出;末尾问用户要不要细化 / 跳过 / 改判 / 切对话逐条过
