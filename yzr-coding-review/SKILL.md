...
description: |
  Use this skill when you need to 和用户一起 review 一段现有代码——
  审视代码合理性 + 找重构机会:语言中立,聚焦设计层判断
  (而非正确性 / 机械检查)。
  默认对话式给出结论(有没有优化空间 + 按严重度的要点),不主动改文件;
  要报告存档 / 大范围体检时产出分级报告;发现多可切逐条过。
  触发: 用户说"review / 重构 / 看看这段代码合不合理 / 陪我一起 review /
  这段设计有没有问题";连续写完一批代码后由用户或 agent 主动询问
  是否需要巡检。
  不适用: 执行用户已给明确改法的单点修改(如"把 data1 改成 user_data") /
  加新功能 / 改 bug / 性能调优(profile / benchmark 专项) / 安全审计 /
  lint 等确定性机械检查 / 重写 / 单步问询(解释代码)。
metadata:
  author: Zuoru YANG
...
- **对话式分析回答**(默认): 结论先行——一句话"有没有优化空间"+ 按严重度的要点(位置 + 场景·卡片 + 理由 + 建议);发现多时的路由见 Step 5
- **报告形态**(用户明确要报告 / 大范围体检): Markdown 报告,结构见 `references/report-template.md`,分轻量 / 完整两档
- **逐条过**(发现多、用户想逐条决策): 按严重度从高到低逐条呈现,等用户确认 / 改判 / 跳过再下一条;结束后按需汇总
...
4. **产物留在对话内**: 用户明确要入 wiki 时,转交 `yzr-outline-wiki` skill 处理
...
按 `references/report-template.md` 两档输出;严重度查 `references/severity-rubric.md`;末尾问用户要不要细化 / 跳过 / 改判 / 切对话逐条过。
