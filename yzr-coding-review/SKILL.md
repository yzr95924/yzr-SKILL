...
结构模板与切换规则见 [Step 4: 形态路由](#step-4-形态路由)–[Step 6: 报告形态](#step-6-报告形态),此处不重抄。
...
开头先给一段**整体理解**(口径同 [Step 5: 对话式分析回答（默认）](#step-5-对话式分析回答默认) 第 1 步,不超过一段),再按 `references/report-template.md`
