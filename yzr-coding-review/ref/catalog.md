...
- **严重度**: 参数 > 5 个 → Major(对齐 rubric);参数簇在 ≥ 3 个函数出现 → Major(对齐 rubric)
...
- **信号**: 相同 / 近似代码块在 ≥ 3 处出现或单块 > 5 行(对齐 rubric);同一修改需在多处同步
...
对 lint 有信号但无结论的主题(死代码 / 魔数 / 空 catch / 函数过长),卡片聚焦"信号之后怎么判"。
