...
- **严重度**: 函数 > 50 行 → Major;> 100 行 → Major 必报;局部提取 → Minor。

### Inline Function

- **信号**: 函数体与函数名一样清晰(几行 + 名字足够);函数被多层嵌套包装(过度抽象)。
- **方案**: 函数体替换到所有调用处;调用处保持可读。
- **严重度**: 多数 Minor;若广泛调用但调用处语境不同可能引入混乱 → 不抽。
...
- **方案**: 用表达式直接替换变量引用。
- **严重度**: 多数 Nitpick;若变量名误导性更强 → 改名而非 Inline。

### Introduce Parameter Object

- **信号**: 多处函数签名带同一组参数(参数簇);参数列表 ≥ 4 个且常一起出现。
- **方案**: 把参数簇封装成结构 / 数据类;所有调用处改为传该对象。
...
- **方案**: 替换函数体为新算法;逐步替换比一次性改安全(可对比测试)。
...
- **严重度**: 职责数量 ≥ 3 → Major;字段混搭无共同语义升 Major。
...
- **严重度**: 多数 Major;≥ 2 个子类有共同初始化 → Major。
