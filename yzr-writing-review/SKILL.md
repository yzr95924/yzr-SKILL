...
把“文字 review”约束成**发现可追溯（每条映射 catalog 规则）、结论可执行**的产物：默认
对话式分析回答，按需出报告或逐条过。
...
- 可选**参照输入**（SSOT 检查用，任意形态：粘贴文本 / 文件路径 / 目录，与 wiki skill
  解耦）；被审目标是多文件时文件集内部互为参照输入，其余情况无参照输入就只审文档内，
  不臆测外部标准

**输出**：三种产物（对话式分析回答 / 报告形态 / 逐条过），按用户意图路由；各形态定义、
结构与切换规则见 [Step 4](#step-4-形态路由)–[Step 6](#step-6-报告形态)，此处不重抄。

## 执行原则

1. **不主动改文件**：产出是结论 / 报告，用户明确点头后才走改写（见[章节](#改写承接确认后)）
2. **每条发现可追溯**：每条发现映射到至少 1 个 catalog 规则名
3. **范围收敛**：只审 catalog 收录的场景（跨文档 SSOT 仅参照输入存在时启用）；不越界到
   事实核查与翻译（发现"事实存疑"时指出位置让用户人工核对，不展开验证）；机械可判定项
   （错别字 / 格式 / 标点）不占主表，并入"不报告项"归并处理
...
- **洁癖但克制**：高标准不等于凑数：琐碎发现合并报或放入"不报告项"，
  主表保信噪比
...
必读 `ref/catalog.md`（规则清单 + AI 腔指纹 + 两个条件节：跨文档 SSOT 仅参照输入存在
时启用，指令文档按被审对象类型启用）。

### Step 3: 走 catalog 补齐

可枚举的指纹先跑机械扫描拿候选：`python3 tools/scan_fingerprints.py <目标文件 | 目录>`（从 skill 根运行）
（命中按对应规则处置，元提及 / 引文由审查者人工豁免；收录哪些模式以脚本 docstring
的契约为准）。随后 LLM 用 catalog 规则补齐发现；每条映射到 ≥ 1 个规则名。跨文档 SSOT
只在有参照输入时启用；无参照输入时在结论里明确"本次只审文档内"。

### Step 4: 形态路由

默认进 **对话式分析回答**（Step 5）；用户明确要报告或大范围体检（多文件 / 遗留文档）时进 **报告形态**（Step 6）；形态可中途切换。
...
4. **收尾问询**：发现多时问"逐条过一遍还是出一份报告存档"；逐条过按文档位置顺序
   逐条呈现，每条等用户表态：
   - **确认**：记为"接受"，下一条
   - **改判**：按用户意见修正内容，下一条
   - **跳过**：记为"跳过"，下一条
...
`ref/report-template.md` 两档输出；
末尾问用户要不要细化 / 跳过 / 改判 / 切对话逐条过 / 进入改写。

## 改写承接（确认后）

用户对发现项点头后进入改写：**对每条接受的发现，执行对应 catalog 规则的修法**。
用户显式说"直接改，不用审"时跳过 review，静默对照 `ref/catalog.md` 定位问题
（不出结论），再按修法改；输出说明以一句对原文的精简理解开头（计入 3 行说明额度）。
**未确认前永不改写**。
...
> 1 节铺背景需求，3 节用对比表比较三方案的延迟 / 成本，4 节补权限设计并参照部署文档核对。
>
> 基于这个理解，有 3 个要点（按文档位置）：
>
> 1. design.md 2.1 节 `冗余重复`：整段换说法复述 1 节的背景，
>    与 1.3 节重复
> 2. design.md 3 节 `论证缺陷`：选型结论说"方案 A 最优"，
>    但 3.2 节对比表里方案 B 延迟更低，结论与论据打架
> 3. design.md 4 节 `内容重复`：与参照的部署文档里权限说明
>    逐字重复，应删并指向 SSOT
>
> 要逐条过一遍，还是出一份报告存档？
...
> **1/3** — design.md 2.1 节 `冗余重复`：整段复述 1 节背景，
> 建议删并保留 1 节那版。接受 / 改判 / 跳过？

用户：接受。
skill：记下，下一条：**2/3** …（循环至汇总）
