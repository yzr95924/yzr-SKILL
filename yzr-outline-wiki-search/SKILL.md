...
description: 用户要在 Outline Wiki 工作区里搜文档或读文档时使用——按关键词全文
  ...
  归档，走 yzr-outline-wiki-upload）；**不**用于配置 outline MCP（走 yzr-outline-wiki-setup）；
...
写 / 编辑 / 上传相关操作由 `yzr-outline-wiki-upload` SKILL.md
负责；MCP 配置由 `yzr-outline-wiki-setup` SKILL.md 负责。
...
- **写 / 编辑 / 上传 outline 文档**（含图片附件 / @mention / 评论 /
  ...
  `yzr-outline-wiki-upload` SKILL.md
...
- 写操作前"先 read 一下"——这是 yzr-outline-wiki-upload 的"读 - 改"流程前置
...
覆盖范围，由 `yzr-outline-wiki-upload` 负责）。
...
1. **写前先搜（Read-First 协作）**
   - 本 skill 自己**只读**，但常作为 yzr-outline-wiki-upload 的"写前 search"前置
...
3. **文档 ID 无效** — 用 search 重新定位拿新 ID；别凭缓存 ID 调用（文档可能
   已归档 / 重命名）
4. **认证失败（401 / 403）** — `yzr-outline-wiki-setup` 范畴（key 过期 / OAuth
   失效），先重启或重跑配置脚本
5. **MCP 未启用 / 连接被拒** — 走 `yzr-outline-wiki-setup` 排查配置
...
`<token>` 位置看 `yzr-outline-wiki-setup` 当时的 scope：
...
```text
1. 确认 mcp__outline__* 工具已在 session 注册（否则先走 yzr-outline-wiki-setup）
...
5. 若用户后续要"改这篇"，明确告知下一步走 yzr-outline-wiki-upload
```
...
2. 列出已有同名 / 相似标题文档
3. 若无 → 提示用户可走 yzr-outline-wiki-upload 创建
4. 若有 → 提示用户走 yzr-outline-wiki-upload 的"编辑已有文档"流程（不新建重复）
```
...
- **不在本 skill 范围内**：
  - 文档风格（`yzr-outline-wiki-upload/references/doc_style.md`）—— 写时参考
  - 风格 checklist（`yzr-outline-wiki-upload/references/style_checklist.md`）—— 写时核对
