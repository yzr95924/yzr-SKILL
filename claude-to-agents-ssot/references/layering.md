...
| 层 | 文件 | 加载时机 | 内容特征 | 预算 |
|----|------|---------|---------|------|
| L1 常驻层 | `AGENTS.md` | 每次 session 必定加载（Claude Code 经薄壳、读 AGENTS.md 的 agent 原生） | 项目身份、核心规约、常用命令、高层结构、跨模块协作摘要 | ≤ 1500 词 |
| L2 记忆层 | `MEMORY/` | `@MEMORY/MEMORY.md` 索引被 import 常驻，正文按需 Read | 跨会话"为什么"、设计决策、踩坑记录 | 索引 ≤ 200 行 |
...
**L1 词数控制**：总词数 ≤ 1500。如果超出，说明内容太详细——把"为什么"类设计决策下沉到 L2（`MEMORY/`），
L1 只保留摘要 + 指向 MEMORY 正文的指针。
