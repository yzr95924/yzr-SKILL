...
OAuth 鉴权或希望手动跑的人：直接 `python3 yzr-outline-wiki-setup/scripts/configure_mcp.py`
走交互模式，脚本会逐项提示。
...
- **Q3 配置范围 (scope)**：选项 `project`（推荐）/ `user` / `local`
  - `project` —— 仅当前项目，配置落仓库根 `.mcp.json`（默认）
  - `user` —— 本机所有项目共享，写 `~/.claude.json`
  - `local` —— 仅本机 + 当前项目，不进 git
  agent 把选择通过 `OUTLINE_MCP_SCOPE=<选择>` 传给脚本；三种 scope 的
  落盘差异详见 `scripts/configure_mcp.py` 的 `choose_scope()`。

## 调用脚本

把上述值放到环境变量，非交互模式调用（scope 来自 Q3，默认 `project`）：

```bash
OUTLINE_MCP_ENDPOINT='<url>' \
OUTLINE_MCP_AUTH_METHOD='apikey' \
OUTLINE_MCP_API_KEY='<key>' \
OUTLINE_MCP_SCOPE='<project|user|local>' \
python3 yzr-outline-wiki-setup/scripts/configure_mcp.py
```
...
4. 未注册则调 `claude mcp add --transport http --scope <scope> outline <url>
...
落盘；详见 `yzr-outline-wiki-upload` SKILL.md
的 curl REST 退路。
...
已废弃。现行流程走 `claude mcp add --scope <project|user|local>`：
project（默认）落到仓库根 `.mcp.json`，user / local 落到
`~/.claude.json`（分别是全局段与项目段），与
