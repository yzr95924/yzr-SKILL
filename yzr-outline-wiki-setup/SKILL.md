...
description: 用于把 Outline Wiki 内置 MCP 服务接入 Claude Code CLI（落盘到
  ...
  文档（走 yzr-outline-wiki-search / yzr-outline-wiki-upload）；**不**用于 Notion /
...
搜 / 读 / 写 / 编辑的具体文档操作，分别由 `yzr-outline-wiki-search` SKILL.md
和 `yzr-outline-wiki-upload` SKILL.md 负责。
...
- **搜 / 读 outline 文档**——走 `yzr-outline-wiki-search` SKILL.md
- **写 / 编辑 outline 文档**（含图片附件 / @mention / 评论 / Collection 管理
  / 移动 / 删除）——走 `yzr-outline-wiki-upload` SKILL.md
- **非 Outline Wiki 的知识库**（Notion / Confluence / Obsidian / GitHub Wiki）
- **配置已成功、只是个别调用报错**——按 yzr-outline-wiki-search / yzr-outline-wiki-upload
  的"故障排查"小节定位
...
跑本 skill 时必须遵守的硬约束（详细论证见右侧段落，别只读本节）：

| 原则 | 详细论证 |
| --- | --- |
| **唯一受控入口**——agent 不直接调 `claude mcp add`、也不直接编辑 `~/.claude.json` 或 `.claude/settings.local.json` | §工作流 / 受控入口约束 |
| **重启必须由用户执行**——脚本不会自己重启 Claude Code | §设计决策（重启硬约束） |
| **scope 三选一不可混用**——`project` / `user` / `local` 落盘位置不同 | §Scope 选择 |
| **诊断优先级**——握手失败 → 401 / 403 → 连接被拒 → 未注册 → 重启后缺失 | §故障排查 |
...
1. **检测当前状态**：先检查 `mcp__outline__*` 工具是否已在 session 注册
   - 若是 → 已配置完成，提示用户直接走 yzr-outline-wiki-search / yzr-outline-wiki-upload
   - 若否 → 进入引导配置流程
...
python3 yzr-outline-wiki-setup/scripts/configure_mcp.py
```

执行成功后，**必须重启 Claude Code 当前会话**，新工具才会出现在
`mcp__outline__*` 列表中。

OAuth 鉴权或希望手动跑：直接 `python3 yzr-outline-wiki-setup/scripts/configure_mcp.py`
走交互模式，脚本会逐项提示。
...
2. **MCP 未启用（连接被拒）**
   ...
   - 自托管实例需管理员在控制台开启
3. **初始化握手失败：响应非 JSON-RPC（端点统一返 200 / 0-body 空响应）**
   - **典型场景**：用户的 endpoint 走了内网穿透 / 反向代理盒子（典型如
     `*.ddnsto.com`），**HTTP 端口对所有路径都返占位 200 + `Content-Length: 0`**；
     真正的 MCP 只在 **HTTPS 443** 才透到上游
   - **判别**：手工 `curl -X POST https://<同 host>:443/mcp`（带 initialize
     body）是否立刻拿到 SSE + JSON init 响应；若 HTTPS 一通即确诊
   - **修**：把 endpoint 改写为 `https://<host>/mcp` 后重跑 `scripts/configure_mcp.py`
   - **关键提示**：这种 middlebox 的 HTTP 端口**永远**返空 200，与鉴权是否
     对、客户端主机是否在 ddnsto 控制台被"标记"无关——标记只让 HTTPS 路径生效
   - **深诊断背景**（为什么换鉴权 / 路径 / Host 头都帮不上忙、协议层
     middlebox 是怎么吃 HTTP 流量的）见
     [`references/onboarding.md`](references/onboarding.md) §常见问题项 3
4. **`claude mcp get outline` 返回 "未找到"**
   - 配置写入失败。常见原因：
     - 环境变量没传到子进程（脚本只读 `os.environ`，shell 没 export）
     - `claude` CLI 没登录（提示用户先 `claude login`）
     - 磁盘写入失败（检查 `~/.claude.json` 权限）
5. **重启后 `/mcp` 仍看不到 outline**
...
6. **已 Connected 但写大文档被 auto-mode 拦（白名单缺失）**
   - **症状**：MCP 状态 ✓ Connected，agent 也确认能调工具，但写
     大文档（≥ 3000 字符，如 `update_document` / `create_document` /
     `create_attachment`）时偶发 false-positive 拦截
   - **Why + 修 + 关键禁忌 + 退路**：见
     [`references/onboarding.md`](references/onboarding.md) §常见问题项 8
     （含 15 条工具白名单 JSON 片段、为什么不写到用户级 settings、
     curl REST API 退路）
...
- `yzr-outline-wiki-search` SKILL.md — 配套：搜 / 读 outline 文档
- `yzr-outline-wiki-upload` SKILL.md — 配套：写 / 编辑 outline 文档
