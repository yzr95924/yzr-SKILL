#!/usr/bin/env python3
"""
Step 6 覆盖率验证：比对原 CLAUDE.md 与迁移后的 AGENTS.md + .qoder/rules/ + 薄壳 CLAUDE.md，
...
局限（诚实声明）：
    ...
    - flag 不等于丢失：可能是去品牌改写 / 合理下沉到 rule 文件。输出里会给每条 flag
      "最佳匹配位置"，辅助判断。
"""
...
def collect_targets(root: Path) -> List[Tuple[str, int, str, frozenset]]:
    ...
    if rules_dir.is_dir():
        for r in sorted(rules_dir.glob("*.md")):
            candidates.append((f".qoder/rules/{r.name}", r))
...
def coverage(root: Path, threshold: float, min_tokens: int) -> Tuple[List[str], int, int]:
    ...
    if not targets:
        lines.append("[硬错误] 目标侧无文件：AGENTS.md / CLAUDE.md / .qoder/rules 都不存在。")
        return lines, -1, 0
    ...
    lines.append("目标：AGENTS.md + .qoder/rules/ + 薄壳 CLAUDE.md")
    lines.append(
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Step 6 覆盖率验证")
    ...
    if flag_count == 0:
        ...
        print("结论：有 flag——逐条确认是「真丢失」还是「去品牌改写 / 下沉到 rule」。")
    return 0
