...
| 字段 | 类型 | 必填 | 说明 |
|---|---|---|---|
| `target` | string（绝对路径 **或** `~/...` home-relative 形式） | 是 | 推荐 `~/src/<source-name>` 形式以跨主机可移植；**也接受** `readlink -f <symlink>` 输出的绝对路径（兼容 0.14.0 前的 anchor）。lint 在判定前统一 `Path(target).expanduser()` 展开——绝对路径展开后不变，`~/...` 展开为 `$HOME/...` |
...
> 进 git，但**不**必须是机器相关绝对路径——推荐写 `~/src/<source-name>` 形式
...
当 `target` 字段经 `Path(target).expanduser()` 展开后**在 git 仓内**
