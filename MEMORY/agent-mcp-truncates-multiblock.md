---
name: agent-mcp-truncates-multiblock
description: 部分 agent 的 MCP 客户端只呈现 tool result 的首个 text content block、丢弃其后；Outline 的 fetch(document) 服务端返 2 个 block（元数据 + 正文），正文段被截，读正文走 REST POST /api/documents.info 旁路。2026-07-01 curl 直连 /mcp 实测 num_blocks=2 确认。临时，待 agent 完整支持多 block 后撤销。
metadata:
  type: project
---
...
但 **部分 agent 的 MCP 客户端只采纳首个 text content block、丢弃其后所有 block**——
所以 agent 里 `fetch` 只看得到 block[0] 元数据。错在 agent 消费侧，**不是** Outline
server 问题，也不是 outline-wiki 安装引入的（agent MCP 配置 `mcpServers.outline` 里
outline 是标准 `type: http` 直连 endpoint，无 bridge）。

> **上游归属（重要，别引错）**：该 agent 这个"只取首个 content block"的行为，社区相关
> tracking 有 `anthropics/claude-code#1804`，但 **#1804 标题 / 正文明确是针对
> `isError: true` 的错误响应**（且 issue 状态 closed）；成功响应多 block 被截是同源
> 表现或关联问题，**未必就是 #1804 本身**。引用时描述"agent 多 content block 处理
...
- 元数据仍用 MCP `fetch`（block[0] 在 agent 里正常可见）；`list_documents` 返单 block
