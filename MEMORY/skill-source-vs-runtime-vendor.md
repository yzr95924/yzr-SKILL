...
仓库内每个 SKILL 目录（`yzr-sys-design-doc/` / `yzr-skill-creator/`）是**单一事实源（SSOT）**。
...
**典型结构**（yzr-sys-design-doc 为例）：

```text
/home/zryang/my_SKILL/yzr-sys-design-doc/                 # ← 代码仓源（SSOT，编辑这里）
/home/zryang/my_SKILL/.agents/skills/yzr-sys-design-doc/  # ← vendored 副本（npx install）
/home/zryang/my_SKILL/.claude/skills/yzr-sys-design-doc   # ← 软链 → 上面那个
```

- `ls -la .claude/skills/yzr-sys-design-doc` → `lrwxrwxrwx ... -> ../../.agents/skills/yzr-sys-design-doc`
- `file .agents/skills/yzr-sys-design-doc` → `directory`（不是软链，是真目录）
...
- **不要**把代码仓的修改当成"已生效"——agent 加载的是 vendored 副本，源改完不
...
```bash
# 1) 确认软链/真目录结构
ls -la .claude/skills/yzr-sys-design-doc
file .agents/skills/yzr-sys-design-doc

# 2) 比对源 vs 当前 vendor
diff -q yzr-sys-design-doc/SKILL.md .claude/skills/yzr-sys-design-doc/SKILL.md
diff -q yzr-sys-design-doc/assets/template-full.md .claude/skills/yzr-sys-design-doc/assets/template-full.md

# 3) 同步源到 vendor（单向）
rsync -a --delete yzr-sys-design-doc/ .claude/skills/yzr-sys-design-doc/
# 注意：--delete 会清掉 vendor 独有的文件，先 dry-run 一次
rsync -a --delete --dry-run yzr-sys-design-doc/ .claude/skills/yzr-sys-design-doc/
```
