...
> [`yzr-llm-wiki-management/references/paper-wiki-profile.md`](../yzr-llm-wiki-management/references/paper-wiki-profile.md) §7。
>
> **重写背景**：2026-06-29 decoupling refactor 之前，本文 §2 / §3 / §4 / §7 多处
> 把 `yzr-llm-wiki-management` 当作 `--full` 模式的设计驱动；重构后改为"基于可识别的
...
yzr-llm-wiki-management）都会采用这个 layout。producer 描述这个 layout 时不
...
- `yzr-llm-wiki-management/references/paper-wiki-profile.md` §7 — **消费方侧的集成契约 SSOT**（本文 producer 侧 SSOT 的对端）