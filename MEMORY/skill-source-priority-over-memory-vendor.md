...
**根因**：用户最终通过 `npx skills add` 把 `gemini-paper-summary/` / `yzr-outline-wiki/` 等子目录分发出去，**分发包只含 SKILL 目录内容**（
`SKILL.md` + `assets/` + `scripts/` + `references/`），**不含** `MEMORY/` 也不含仓库的"为什么"记录。**所以 MEMORY 里的"影响 SKILL
效果"的内容如果不显式落到 SKILL 目录，npx 装出去的版本就会丢这些规则。**

**优先级排序**（从高到低）：

1. **SKILL 源**（仓库根 `gemini-paper-summary/` / `yzr-outline-wiki/` / `design-doc-edit/` / `yzr-skill-creator/`）——
