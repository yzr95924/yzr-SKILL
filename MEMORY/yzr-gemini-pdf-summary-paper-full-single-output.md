---
name: yzr-gemini-pdf-summary-paper-full-single-output
description: yzr-gemini-pdf-summary paper --full 模式从"双产物 quick+full"翻为"单产物 full"的设计决策与边界（2026-07-07 翻面）
metadata:
  type: project
  supersedes: gemini-paper-summary-full-mode-design.md §1 (D1) + §2（双产物 why）
---

# yzr-gemini-pdf-summary paper --full 单产物设计（2026-07-07 翻面）

## Why

`yzr-gemini-pdf-summary` 的 `paper --full` 模式原设计是"单次调用同时产 quick summary +
...
1. **产品定位已分家**。`yzr-gemini-pdf-summary-manual-whitepaper-full-design.md` D1 已经把
   ...
   产品线。
2. **下游消费方不需要"对齐"**。`yzr-llm-wiki-management` 的 ingest_diff 只读 full.md 当
...
未来给 `yzr-gemini-pdf-summary` 加内容或改模板时：
...
3. **eval / debug 友好**：单产物调用出错时排错路径短（一份产物 + 一次调用），
   双产物出错要判断是哪一份、哪一次调用导致——已经吃过这个亏（98 页白皮书 1 次调用 →
   35%+ 章节截断的同类问题，与双产物不同但都属于"一次调用 → 多产物"的复杂度累积）

## 关联

- [[yzr-gemini-pdf-summary-manual-whitepaper-full-design]] — 本翻面的前置（manual / whitepaper
