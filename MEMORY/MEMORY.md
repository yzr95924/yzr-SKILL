...
`@MEMORY/MEMORY.md` 引入（自动展开 `@import` 的 agent 读入全文，不展开者自行 Read 本文件）；
条目正文按需 `Read`（`MEMORY/<slug>.md`，同级）；写入与治理流程唯一归 `yzr-memory-management`
skill。新条目追加到本文件末尾即可，无需同步副本——只活这一份，AGENTS.md 单行引用负责把"指针"
挂到 L1。
