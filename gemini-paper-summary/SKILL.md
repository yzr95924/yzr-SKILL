...
| 关注点 | ✗ | `--focus "重点关注实验部分"` 之类，会追加到 prompt；**full 模式行为不同**——不是末尾追加 "启发 / 追问" 段，而是在对应的 PDF 原生章节下注入 `### 用户关注点: <focus>` 子节（详见 [`references/full-mode-contract.md`](references/full-mode-contract.md) 关键行为护栏） |
...
| 全量抽取模式 | ✗ | `--full`，单次调用同时产 quick summary + 全文级抽取**两份**产物；**产出 layout 强制 raw-compatible**——`--output <wiki_root>`，产物落到 `<wiki_root>/raw/papers/<slug>.quick.md` + `.full.md`(full 自包含,无 PNG 配套);quick 模式另产生 `<wiki_root>/raw/assets/<slug>/fig-NN.png`;完整契约见 [`references/full-mode-contract.md`](references/full-mode-contract.md) |
...
2. **结构化总结，不是复述**
   - 按 outline 风格模板的**主线顺序与命名**输出（开篇表 → 团队 item →
     `## 3 句话总结` → 背景与动机 → 方法设计 → 代表性实验结果 → 业务启示 & 价值 → 局限与未来工作），精炼优先；目标字符数见
     `assets/prompt-template.md`（**单一来源**，勿在正文写死数值）
...
> （**代价 SSOT** 见 [`references/full-mode-contract.md`](references/full-mode-contract.md) §关键纪律 #4 Stage 2 是哑参数条）。
...
完整契约（接口约定 / 调用形式 / 产物 layout / 关键纪律 / 关键行为护栏 / 反模式 / 故障排查）已搬至 [`references/full-mode-contract.md`](references/full-mode-contract.md)。

**只看这里就够的 5 条要点**：

1. **接口核心**：单次调用同时产 quick summary + PDF→Markdown 全量转写两份产物，layout 强制
   raw-compatible（`--output <wiki_root>` 视作 wiki 仓根，产物落到
   `<wiki_root>/raw/papers/<slug>.{quick,full}.md`）。**不要**用 `--full` 把产物落任意目录——
   那是 `--extract-figures` 的职责
2. **2026-06-30 第二轮翻面**：full 模式不落 PNG、不跑 Stage 2、不写 `![图 N](...)` 引用
   （自包含：架构图 ` ```mermaidjs ` block 画、数据图转表、装饰图省略）。要 PNG 跑 quick 模式
   （默认带图）
3. **必须一次调两份**：拆成两次调用会因 prompt 不一致导致两份对不上；强行只产 full 也违反设计本意
4. **不消费 `--refine-figures` / `--thumbnail`**：这两个 flag 在 full 模式是哑参数
   （full 不落 PNG），脚本 stderr INFO 提示
5. **`--focus` 行为**：quick 模式走 FOCUS_INJECTION（末尾追加 "## 启发 / 追问" 段），
   full 模式走 FOCUS_INJECTION_FULL（注入到对应 PDF 原生章节下，形式为
   `### 用户关注点: <focus>` 子节）。脚本按 `template="full"` 自动切换，
   契约测试见 `eval/evals.json` test #2 assertion 2

> **其余细节**（5 项后置内容自检阈值的具体参数、Output token 上限 SSOT、full 不重写契约、
> 反模式 / 故障排查等）——一律走 `references/full-mode-contract.md`。
>
> **为什么把契约搬到 references/ 而不是留在 SKILL.md？**`--full` 模式是相对边缘的产品功能
> （v3.x 增量，2026-06-30 刚翻面），agent 触发频率远低于 quick 模式。SKILL.md 直接承载 134 行
> 会推字到 5000 词上限；按需加载的 references/ 在 npx 分发后只被需要 `--full` 的少数场景读到，
> 主流程不受影响。
