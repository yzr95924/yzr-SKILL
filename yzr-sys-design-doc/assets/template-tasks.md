...
> 设计要改时先修订设计文档，再回本文件同步受影响任务（见「问题反馈与设计变更」循环纪律）。

## 执行者操作指引（拿到本文件先读）

你（执行者，人或 agent）按以下循环操作本文件：

1. **认领**：从「任务总表」挑一个状态"未开始"且依赖已完成的任务，状态改"进行中"，
   在「任务执行记录」对应小节写下开始记录（日期 + 执行者）。
2. **回读设计**：按任务行的"设计落点 / 验收场景"指针读设计文档对应章节——本文件**不含**
   设计细节，不回读设计文档不许动手。
3. **执行 + 刷新**：每次有进展，在「任务执行记录」该任务小节追加一条执行记录（日期 / 内容 / 偏差 / 遗留）；
   状态变化时同步更新「任务总表」的"状态"列与元信息的"最近更新"。
4. **完成**：状态改"待验收"；验收者对照验收场景逐条验收、写结论，状态改"已验收"。
5. **遇到问题**：无法继续 → 状态"已阻塞"并在「问题反馈与设计变更」登记；发现**设计本身要改** →
   走「问题反馈与设计变更」循环纪律，不许绕过设计改实现。

你只能改三处：「任务总表」的"状态"列、「任务执行记录」的执行记录（只追加、不改历史）、
「问题反馈与设计变更」两张表。
任务定义（编号 / 内容 / 指针）由设计负责人维护——你觉得任务拆得不对，同样走「问题反馈与设计变更」登记。
