...
description: |
  ...
  Markdown 本地文件（full 档覆盖需求层 / 方案层 / 落地层 + DFX），并配套生成实施任务书
...
- **体量匹配规模**：小改动套 full 模板 = 评审者读十几个章节找一个字段变更；大改动用 lite =
  漏影响面与回滚。判档按**工作流**第 1 步的档位表（[章节](#工作流)），用户明示档位时听用户的
...
- **量化且标口径**：能用数字就不用形容词——“超卖率千分之三” > “经常超卖”，“P99 < 200ms” > “低延迟”。
  ...
  定不了的进假设清单
- **口径一致**：同一规则 / 数字 / 术语全文只有一个说法——规则在“功能规格与约束”节单点
  陈述，他处引用不重抄；同一概念前后同名（别“冷静期”又叫“冻结期”）；跨节数字必须相同
  （“功能规格与约束”写 TTL 30 分钟，“详细设计”就不能是 1 小时）。评审者抓到两处口径
  打架，会对全文真实性打折
...
5. **生成任务书（仅 full 档）**：按 `assets/template-tasks.md` 从功能点拆解与详细设计拆出任务，
   ...
   进度 / 问题 / 设计变更按任务书模板“问题反馈与设计变更”的循环纪律流转；lite 档跳过本步
6. **自检**（交付前逐条对照）：
   ...
   - 同一规则 / 术语 / 数字跨节口径一致（规则以“功能规格与约束”为单点，他处引用不重抄）（full 档）
   - 除元信息“评审人：待定”外，全文无裸 TBD / 待补充——未决项集中在开放问题节（full 档）
   - DFX 各子项（枚举见 [template-full.md](assets/template-full.md) DFX 节）均显式回应，不适用项有依据（full 档）
   ...
   - 影响面覆盖模板影响面节枚举的全部维度（full 档“兼容与影响面”/ lite 档“影响面”），
     无影响的维度写“无”+ 依据
